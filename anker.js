...
async function starts() {
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			const BotWea2 = ["08990542731@s.whatsapp.net"] // ubah aja gapapa
			...
			mess = {
				...
				only: {
					...
					userB: `*Hallo Kak* ${pushname2} *Kayanya Kamu Blm Terdaftar Sebagai Teman FDL BOT*\n*Ketik ${prefix}daftar nama|umur*\n*Contoh ${prefix}daftar FDL GANZ|17*`,
			...
			const ownerNumber = ["628990542731@s.whatsapp.net"] // owner number ubah aja
		...
		case prefix+'daftar':
         ...
         case prefix+'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    dms.sendMessage('628990542731@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case prefix+'map':
				...
				case prefix+'clearall':
					...
					reply(`*_Sukses delete all chat BOT_*`)
					break
                                case prefix+'bcgc':
					dms.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case prefix+'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *FDL BOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
